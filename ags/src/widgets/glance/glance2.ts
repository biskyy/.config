...
const GlanceWeather = () => {
    ...
    child: Widget.Box({
      ...
      children: [
        ...
        Widget.Label({
          ...
          label: Weather.bind("feels_like").transform(
  ...
  return Widget.EventBox({
    ...
    on_primary_click: () =>
      Utils.execAsync([
        "xdg-open",
        `https://openweathermap.org/city/${Weather.city_id}`,
      ]),
            ...
            Widget.Box({
              ...
              children: [
                ...
                Widget.Label({
                  ...
                  label: Weather.bind("temperature").transform(
                    (temp) => `${(temp - 273.15).toFixed(1)}°C`
                  ),
                }),
              ],
...
const GlancePower = () =>
  Widget.Box({
    ...
    children: [
      Widget.Box({
        orientation: Gtk.Ori2entation.VERTICAL,
        homogeneous: true,
        children: [
          Widget.Label({
            class_name: "mode",
            vpack: "end",
            hpack: "end",
            label: PowerProfiles.bind("mode").transform(
          ...
          Widget.Label({
            ...
            1000,
            (widget) =>
              (widget.label = `uptime: ${to_timestamp(
                parseInt(Utils.readFile("/proc/uptime"))
              )}\n`)
          ),
        ],
      }),
  ...
  Widget.Box({
    ...
    children: [
      Widget.Box({
        ...
        children: [
          Widget.Icon({
            icon: Network.bind("primary").transform((primary) =>
              primary === "wifi"
        ...
        child: Widget.Box({
          ...
          setup: (widget) => {
            widget.hook(Bluetooth, (widget) => {
              widget.children = Network.wifi.access_points.map((device) =>
                Widget.EventBox({
                  hexpand: true,
                  class_name: "entry",
                  child: Widget.Box({
                    ...
                    children: [
                      ...
                      Widget.Label({
                        class_name: "alias",
                        label: truncate(device.ssid || "Unknown", 18),
                      }),
                    ],
                  }),
                })
          ...
          Widget.Label().hook(
            ...
            (widget) => {
              widget.label = truncate(
                Bluetooth.connected_devices[0]?.alias || "Disconnected",
                28
              );
            },
        ...
        child: Widget.Box({
          ...
          setup: (widget) => {
            widget.hook(
              Bluetooth,
              (widget) => {
                widget.children = Bluetooth.devices.map((device) =>
                  Widget.EventBox({
                    hexpand: true,
                    class_name: "entry",
                    child: Widget.Box({
                      ...
                      children: [
                        ...
                        Widget.Label({
                          class_name: "alias",
                          label: truncate(device.alias || "Unknown", 18),
                        }),
                      ],
                    }),
                  })
  ...
  Widget.Box({
    ...
    children: [
      Widget.Label({
        ...
        return (widget.label = `${String(datetime.getHours()).padStart(
          2,
          "0"
...
const GlanceSettings = () => {
          ...
          setup: (widget) => {
            widget.hook(Bluetooth, (widget) => {
              if (Bluetooth.connected_devices.length > 0) {
                widget.value =
                  Bluetooth.connected_devices[0].battery_percentage / 100;
                widget.rounded = true;
              } else {
                widget.value = 0.0;
                widget.rounded = false;
  ...
  return [GlancePages, GlanceControls];
};
  ...
  Widget.Box({
    ...
    children: [
      Widget.Box({
        spacing: 20,
        orientation: Gtk.Orientation.VERTICAL,
        children: [GlanceWeather(), GlancePower()],
      }),
      Widget.Box({
